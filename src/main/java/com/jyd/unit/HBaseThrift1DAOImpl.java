...
public class HBaseThrift1DAOImpl extends HBaseThrift1DAO {

	public HBaseThrift1DAOImpl() {
		super();
	}

	public HBaseThrift1DAOImpl(String host, int port) {
		super(host, port);
	}
	...
	@Override
	public void update(String table, String rowKey, boolean writeToWal, Map<String, String> fieldNameValues,
	...
	public void createTable(String table) throws IOError, IllegalArgument, AlreadyExists, TException {
		...
		cd.setName(decode(wrap(ColumnFamily)));
		List<ColumnDescriptor> cdList = new ArrayList<ColumnDescriptor>();
		cdList.add(cd);
		// create 'department','master'
		client.createTable(wrap(table), cdList);
	}

	@Override
	public void update(String table, String rowKey, boolean writeToWal, String fieldName, String fieldValue,
			Map<String, String> attributes) throws IOError, TException {
	...
	@Override
	public void deleteCells(String table, String rowKey, boolean writeToWal, List<String> columns,
			Map<String, String> attributes) throws TException {
		List<Mutation> mutations = new ArrayList<Mutation>();
	...
	@Override
	public void deleteCell(String table, String rowKey, boolean writeToWal, String column,
			Map<String, String> attributes) throws TException {
	...
	@Override
	public int scannerOpen(String table, String startRow, List<String> columns, Map<String, String> attributes)
			throws TException {
		ByteBuffer tableName = wrap(table);
	...
	@Override
	public int scannerOpenTs(String table, String startRow, List<String> columns, long timestamp,
			Map<String, String> attributes) throws TException {
		ByteBuffer tableName = wrap(table);
		List<ByteBuffer> fl = encodeColumns(columns);
		Map<ByteBuffer, ByteBuffer> wrappedAttributes = encodeAttributes(attributes);
	...
	@Override
	public int scannerOpenTs(String table, String startRow, String stopRow, List<String> columns, long timestamp,
			Map<String, String> attributes) throws TException {
		ByteBuffer tableName = wrap(table);
		List<ByteBuffer> fl = encodeColumns(columns);
		Map<ByteBuffer, ByteBuffer> wrappedAttributes = encodeAttributes(attributes);
	...
	@Override
	public List<TRowResult> getRows(String table, List<String> rows, Map<String, String> attributes) throws TException {
		ByteBuffer tableName = wrap(table);
		Map<ByteBuffer, ByteBuffer> wrappedAttributes = encodeAttributes(attributes);
	...
	@Override
	public List<TRowResult> getRowsWithColumns(String table, List<String> rows, List<String> columns,
			Map<String, String> attributes) throws TException {
		ByteBuffer tableName = wrap(table);
		List<ByteBuffer> wrappedRows = encodeRows(rows);
	...
	@Override
	public String iterateResultjson(TRowResult result) {
		Iterator<Entry<ByteBuffer, TCell>> iter = result.columns.entrySet().iterator();
		JSONObject jsonRow = new JSONObject();
		jsonRow.put("total", 3);
		List<JSONObject> cellList = new ArrayList<JSONObject>();
		while (iter.hasNext()) {
			Entry<ByteBuffer, TCell> entry = iter.next();
			JSONObject jsonCell = new JSONObject();
			jsonCell.put(new String(decode(entry.getKey())), new String(entry.getValue().getValue()));
			cellList.add(jsonCell);
		}
		jsonRow.put("rows", cellList);
		return jsonRow.toJSONString();
	}
}