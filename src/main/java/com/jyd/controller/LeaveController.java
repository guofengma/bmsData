...
public class LeaveController extends Controller{
	...
	public void findTypes() throws TException {
		...
		String[] types = { "leave_num","position_num" };      
		List<Long> typeList = new ArrayList<>();           

		// 离职
		Long count1 = Db.queryLong("SELECT count(1) as leave_num FROM hr_employee e,hr_department d where e.department_id=d.id and e.state =1 and date_format(e.update_date,\'%Y%m%d\') = date_format(SYSDATE(),\'%Y%m%d\') and d.store_id=?", storeId);
	...
	public void weekAndMonth() {
		...
		String[] keys = { "leave_num","position_num"   };
	...
	public void day() {
		...
		List<Record> count1 = Db.find("SELECT c.short_name,b.department_name,a.name,d.position_type,a.contact_information,a.effective_date FROM hr_employee a,hr_department b, ba_store c,ba_position_type d WHERE a.department_id= b.id and b.store_id=c.id and a.position_id=d.id and a.state= 1 AND date_format(a.create_date,'%Y%m%d')= date_format(SYSDATE(),'%Y%m%d') AND b.store_id = ? order by a.effective_date DESC ", storeId);
		List<Record> count2 = Db.find("SELECT c.short_name,b.department_name,a.name,d.position_type,a.contact_information,a.effective_date FROM hr_employee a,hr_department b, ba_store c,ba_position_type d WHERE a.department_id= b.id and b.store_id=c.id and a.position_id=d.id and a.state= 0 AND b.store_id = ? order by a.effective_date DESC ", storeId);
		...
		List<Record> count3 = Db.find("select b.position_type ,count(a.id_no) as num from hr_employee a,ba_position_type b,hr_department c where a.position_id=b.id  and a.department_id=c.id and a.state=0 and c.store_id=? group by b.position_type ", storeId);
