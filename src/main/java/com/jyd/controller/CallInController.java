...
public class CallInController extends Controller {

	private StoreService storeS = StoreService.me;

	public void index() {
		render("/contract/callIn.html");
	}
	...
	public void findTypes() throws TException {
		...
		String[] types = { "tuning_in_num", "position_num" };
		List<Long> typeList = new ArrayList<>();

		//
		Long count1 = Db.queryLong(
				"select count(1) from hr_employee e,hr_emp_experience x,hr_department d where e.id= x.employee_id and d.id=e.department_id  and x.type=2  and date_format(x.update_date,'%Y%m%d')= date_format(SYSDATE(),'%Y%m%d')   and d.store_id= ?",
				// "SELECT COUNT(1) FROM hr_to_loan l,hr_department d where
				// l.department_new=d.id and d.state=0 and d.store_id= ?",
				storeId);
	...
	public void weekAndMonth() {
		...
		String[] keys = { "tuning_in_num", "position_num" };
	...
	public void day() {
		...
		List<Record> count1 = Db.find(
				"SELECT c.short_name,b.department_name,a.name,d.position_type,a.contact_information,a.entry_date FROM hr_employee a,hr_department b, ba_store c,ba_position_type d WHERE a.department_id= b.id and b.store_id=c.id and a.position_id=d.id and a.state= 0 AND date_format(a.create_date,'%Y%m%d')= date_format(SYSDATE(),'%Y%m%d') AND b.store_id = ? order by a.entry_date DESC ",
				storeId);
		// List<Record> count1=Db.find("SELECT
		// c.short_name,d.department_name,b.name,e.position_type
		// ,a.new_data,a.update_date " +
		// " FROM bms.hr_emp_experience a,hr_employee b,ba_store c,hr_department
		// d,ba_position_type e where a.employee_id=b.id and b.department_id=d.id and
		// b.position_id = e.id and c.id = d.store_id and a.type = ? order by
		// a.create_date desc " , storeId);
		List<Record> count2 = Db.find(
				"SELECT c.short_name,b.department_name,a.name,d.position_type,a.contact_information,a.entry_date FROM hr_employee a,hr_department b, ba_store c,ba_position_type d WHERE a.department_id= b.id and b.store_id=c.id and a.position_id=d.id and a.state= 0 AND b.store_id = ? order by a.entry_date DESC",
				storeId);
