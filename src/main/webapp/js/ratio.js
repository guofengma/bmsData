...
lineChart = {
		...
		initData:function(div,obj){
			...
			this.option =  {
					...
					lineChart.interval(lineChartObject);
...
// 					}
				...
				invokingAjax:function (lineChartObject,url, data, typeName, startDate, endDate, storeName){
					$.post(url, data, function(response){
						if(response.show_res_code == "0"){
		 					lineChart.process(lineChartObject,response, startDate, endDate, typeName, storeName);
		 				}
				...
				process:function (lineChartObject,response, startDate, endDate, typeName, storeName){	

					lineChartObject.option.title.text = "\n"+startDate+"至"+endDate+storeName+"金额时趋图";
...
line2Chart = {
		...
		initData:function(div,obj){
			...
			this.option =  {
					...
					line2Chart.interval(lineChartObject);
// 					for(var i = 0; i < typeNames.length; i++){
// 						$.post(urls[i], data, function(response){
// 			 				console.log(response);
...
// 					}
				...
				invokingAjax:function (lineChartObject,url, data, typeName, startDate, endDate, storeName){
					$.post(url, data, function(response){
						if(response.show_res_code == "0"){
							line2Chart.process(lineChartObject,response, startDate, endDate, typeName, storeName);
		 				}
		 				if(response.show_res_code == "-1"){
		 					$.messager.alert('提示',response.show_res_error,'info');
		 				}
				...
				process:function (lineChartObject,response, startDate, endDate, typeName, storeName){	
					...
					lineChartObject.option.title.text = "\n"+startDate+"至"+endDate+storeName+"比率时趋图";
					lineChartObject.option.xAxis.data = response.show_res_body.month;
					lineChartObject.option.legend.data.push(typeName);
...
//					            name:'未结清合同',
			        ...
			        barChart.process(barChartObject);
				},
				process:function (barChartObject){	
					...
					barChartObject.option.title.text = storeName+'当天金额情况';
...
//					            name:'未结清合同',
			...
			bar2Chart.process(barChartObject);
		},
		process:function (barChartObject){	
			var storeName = barChartObject.storeName;
			var typeNames = barChartObject.typeNames;
			...
			barChartObject.option.title.text = storeName+'当天比率情况';
			barChartObject.option.xAxis[0].data = typeNames;
			barChartObject.option.series[0].data = data;
...
ratio_principal_barChart = {
		...
		initData:function(div,obj){
			...
			ratio_principal_barChart.process(barChartObject);
		},
		process:function (barChartObject){	
			var storeName = barChartObject.storeName;
...
ratio_interest_barChart = {
		...
		initData:function(div,obj){
			...
			ratio_interest_barChart.process(barChartObject);
		},
		process:function (barChartObject){	
			var storeName = barChartObject.storeName;
			var xAxisData = barChartObject.xAxisData;
