...
callin_lineChart = {
		...
		initData:function(div,obj){
			...
			callin_lineChart.interval(lineChartObject);
				...
				invokingAjax:function (lineChartObject,url, data, typeName, startDate, endDate, storeName,title){
					$.post(url, data, function(response){
		 				...
		 				if(response.show_res_code == "0"){
		 					callin_lineChart.process(lineChartObject,response, startDate, endDate, typeName, storeName,title);
		 				}
...
//					            name:'未结清合同',
		...
		callin_barChart.process(barChartObject);
			},
...
//					            name:'直接访问',
			...
			tuning_in_num_barChart.process(barChartObject);
				},
				process:function (barChartObject){	
					var storeName = barChartObject.storeName;
