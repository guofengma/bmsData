...
leave_lineChart = {
		...
		initData:function(div,obj){
			...
			leave_lineChart.interval(lineChartObject);
				...
				invokingAjax:function (lineChartObject,url, data, typeName, startDate, endDate, storeName,title){
					$.post(url, data, function(response){
		 				...
		 				if(response.show_res_code == "0"){
		 					leave_lineChart.process(lineChartObject,response, startDate, endDate, typeName, storeName,title);
		 				}
...
//					            name:'未结清合同',
			...
			leave_barChart.process(barChartObject);
				},
...
//					            name:'直接访问',
				...
				leave_num_barChart.process(barChartObject);
				},
				process:function (barChartObject){	
					var storeName = barChartObject.storeName;
