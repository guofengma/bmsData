...
noPayCapital_lineChart = {
		...
		initData:function(div,obj){
			...
			this.option =  {
					...
					noPayCapital_lineChart.interval(lineChartObject);
// 					for(var i = 0; i < typeNames.length; i++){
// 						$.post(urls[i], data, function(response){
// 			 				console.log(response);
			 				
// 			 				noPayCapital_lineChart.process(lineChart, response, startDate, endDate, typeName);
// 						});
// 					}
				...
				invokingAjax:function (lineChartObject,url, data, typeName, startDate, endDate, storeName, title){
					$.post(url, data, function(response){
						if(response.show_res_code == "0"){
							noPayCapital_lineChart.process(lineChartObject,response, startDate, endDate, typeName, storeName, title);
		 				}
...
noPayCapital_line2Chart = {
		...
		initData:function(div,obj){
			...
			this.option =  {
					...
					noPayCapital_line2Chart.interval(lineChartObject);
// 					for(var i = 0; i < typeNames.length; i++){
// 						$.post(urls[i], data, function(response){
// 			 				console.log(response);
...
// 					}
				...
				invokingAjax:function (lineChartObject,url, data, typeName, startDate, endDate, storeName, title){
					$.post(url, data, function(response){
						if(response.show_res_code == "0"){
							noPayCapital_line2Chart.process(lineChartObject,response, startDate, endDate, typeName, storeName, title);
		 				}
		 				if(response.show_res_code == "-1"){
		 					$.messager.alert('提示',response.show_res_error,'info');
		 				}
...
//					            name:'未结清合同',
			        ...
			        noPayCapital_barChart.process(barChartObject);
				},
...
//					            name:'未结清合同',
			...
			noPayCapital_bar2Chart.process(barChartObject);
		},
		process:function (barChartObject){
			var title = barChartObject.title;
			var storeName = barChartObject.storeName;
...
uncleared_principalInterest_barChart = {
		storeName:null,
		xAxisData:null,
		series:null,
		legendData:null,
		...
		initData:function(div,obj){
			...
			this.option =  {
					...
					color: ['#3398DB', '#749f83', '#91c7ae','#749f83',  '#ca8622', '#bda29a','#6e7074', '#546570', '#c4ccd3'],
					tooltip : {
						trigger: 'axis',
						axisPointer : {            // 坐标轴指示器，坐标轴触发有效
							type : 'shadow'        // 默认为直线，可选为：'line' | 'shadow'
			...
			uncleared_principalInterest_barChart.process(barChartObject);
		},
		process:function (barChartObject){	
			var storeName = barChartObject.storeName;
			var xAxisData = barChartObject.xAxisData;
